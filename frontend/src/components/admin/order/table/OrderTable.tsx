  ...
  return (
    <div className="w-full space-y-4">
      <OrderTableFilters
        ...
        totalOrdersLength={filteredOrders.length}
      />
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <TableRow key={headerGroup.id} className="border-gray-200">
                {headerGroup.headers.map((header) => (
                  <TableHead
                    key={header.id}
                    className="bg-gray-50 text-gray-900 font-semibold border-gray-200 h-14 px-6 text-base"
                  >
            ...
            {table.getRowModel().rows?.length ? (
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  className="border-gray-100 hover:bg-gray-50/50 h-16"
                >
                  {row.getVisibleCells().map((cell) => (
                    <TableCell
                      key={cell.id}
                      className="text-gray-900 border-gray-100 px-6 py-4 text-base"
                    >
                      {flexRender(
              ...
              <TableRow>
                <TableCell
                  ...
                  className="h-32 text-center text-gray-500 text-lg"
                >
                  No orders found.
                </TableCell>
              </TableRow>
